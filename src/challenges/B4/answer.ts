...
export default function ({ submissions }: {submissions: Submission[] }): MonthSubmission[] {
    const months: {[key: string]: Submission[]} = {};
  
    // Group submissions by month
    for (const submission of submissions) {
      const date = new Date(submission.submittedAt);
      const month = new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), 1)).toISOString();
      if (!months[month]) {
        months[month] = [];
      }
      months[month].push(submission);
    }
  
    // Sort months chronologically
    const sortedMonths = Object.keys(months).sort();
  
    // Create MonthSubmission objects
    const result: MonthSubmission[] = [];
    for (const month of sortedMonths) {
      result.push({
        month,
        submissions: months[month].sort((a, b) => a.submittedAt.localeCompare(b.submittedAt)),
      });
    }
